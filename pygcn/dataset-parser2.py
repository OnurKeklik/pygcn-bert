...
bitflag = int(sys.argv[2]) if len(sys.argv) > 2 else 3
current_idx = 1
idx_dictionary = { }
f1 = open("../data/" + dataset + "/data.cites", "w")
f2 = open("../data/" + dataset + "/data.content", "w")
max_sen_length = 53
max_abstract_length = 423 if bitflag & 2 != 0 else 0
max_title_length = 36 if bitflag & 1 != 0 else 0
...
def write_citation(row):
    ...
    tokens_source_abstract = tokenizer.tokenize(source_abstract[0:2600]) if bitflag & 2 != 0 else []
    tokens_source_title = tokenizer.tokenize(source_title[0:512]) if bitflag & 1 != 0 else []
